...
function hasKeyCI(map, name) {
  if (!map) return false;
  const u = name.toUpperCase();
  for (const k of Object.keys(map)) if (k.toUpperCase() === u) return true;
  return false;
}
  ...
  async getDevices(versions=[3,2]) {
    ...
    for (const d of api_response) {
      ...
      try {
        ...
        const map = props.response || {};
        const hasV3 = hasKeyCI(map, 'REAL_TIME_VITALS');
        const hasV2 = hasKeyCI(map, 'CHARGE_STATUS');
